...
- [ ] AC 1.1: Upon receiving `SIGTERM` or `SIGINT`, `ptimeout` terminates its child command.
- [ ] AC 1.2: `ptimeout` exits gracefully after handling the signal and terminating its child.
- [ ] AC 1.3: No orphaned processes remain if `ptimeout` is terminated while a child command is running.
- [ ] AC 1.4: The exit code of `ptimeout` reflects that it was terminated by a signal (e.g., `128 + signal_number`).
...
- [ ] Task 1: Register signal handlers for `SIGTERM` and `SIGINT`.
  ...
  - **Subtasks**:
    - [ ] Subtask 1.1: Use `signal.signal()` to associate `SIGTERM` and `SIGINT` with a custom handler function.
...
- [ ] Task 2: Implement graceful child process termination in the signal handler.
  ...
  - **Subtasks**:
    - [ ] Subtask 2.1: In the signal handler, iterate through and terminate any active child processes.
    ...
    - [ ] Subtask 2.2: Implement any necessary cleanup before `ptimeout` exits.
