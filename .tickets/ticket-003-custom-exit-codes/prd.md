...
- [ ] AC 1.1: `ptimeout` returns exit code `0` on successful command completion within the timeout.
- [ ] AC 1.2: `ptimeout` returns a specific exit code (e.g., `124` for timeout) when the command times out.
- [ ] AC 1.3: `ptimeout` returns the wrapped command's exit code if the command fails within the timeout.
- [ ] AC 1.4: `ptimeout` returns a distinct exit code for invalid arguments or `ptimeout` internal errors.
...
#### Main Task Structure
- [ ] Task 1: Define `ptimeout`'s custom exit codes.
  ...
  - **Subtasks**:
    - [ ] Subtask 1.1: Identify standard practices or conventions for CLI exit codes (e.g., `124` for timeout).
    ...
    - [ ] Subtask 1.2: Define specific integer values for success, timeout, command failure, and internal `ptimeout` errors.
...
- [ ] Task 2: Modify `ptimeout` to return the defined exit codes.
  ...
  - **Subtasks**:
    - [ ] Subtask 2.1: Adjust the `run_command_with_timeout` function to set the appropriate exit code.
