...
- [ ] AC 1.1: CI/CD workflow includes a job to build and test the `ptimeout` binary on Linux.
- [ ] AC 1.2: CI/CD workflow includes a job to build and test the `ptimeout` binary on macOS.
- [ ] AC 1.3: (Optional, if feasible) CI/CD workflow includes a job to build and test the `ptimeout` binary on Windows.
- [ ] AC 1.4: A set of integration tests are executed against the built binaries to verify core functionality.
...
#### Main Task Structure
- [ ] Task 1: Extend `release.yml` to include a multi-platform build and test matrix.
  ...
  - **Subtasks**:
    - [ ] Subtask 1.1: Modify `.github/workflows/release.yml` to define a `strategy.matrix` for operating systems.
    ...
    - [ ] Subtask 1.2: Adjust the build step to produce binaries for each platform in the matrix.
...
- [ ] Task 2: Implement integration tests for the standalone binary.
  ...
  - **Subtasks**:
    - [ ] Subtask 2.1: Create a dedicated test script (e.g., `tests/binary_integration_test.sh`) for binary testing.
    ...
    - [ ] Subtask 2.2: Integrate the binary integration test script into the CI/CD workflow.
