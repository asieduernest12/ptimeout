...
- [ ] AC 1.1: When `--dry-run` is used, `ptimeout` prints the command it *would* execute, but does not actually run it.
- [ ] AC 1.2: For nested `ptimeout` commands, the output clearly shows the full nested command string.
- [ ] AC 1.3: Piped input scenarios should also display the final command that would receive the piped input.
- [ ] AC 1.4: `--dry-run` should be compatible with other `ptimeout` options (e.g., `-v`, `-r`).
...
#### Main Task Structure
- [ ] Task 1: Add `--dry-run` flag to `ptimeout`'s argument parser.
  ...
  - **Subtasks**:
    - [ ] Subtask 1.1: Modify `ptimeout.py`'s `argparse` configuration to include a `--dry-run` flag.
...
- [ ] Task 2: Implement logic to print the command instead of executing it when `--dry-run` is enabled.
  ...
  - **Subtasks**:
    - [ ] Subtask 2.1: Modify the main execution loop to check for the `--dry-run` flag.
    ...
    - [ ] Subtask 2.2: Ensure nested `ptimeout` commands are accurately represented in the `--dry-run` output.
